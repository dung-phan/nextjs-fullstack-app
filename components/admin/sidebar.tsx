'use client'

import React from 'react'
import { BookOpen } from 'lucide-react'
import { adminSideBarLinks } from '@/constants/admin'
import { DynamicIcon } from 'lucide-react/dynamic'
import Link from 'next/link'
import { cn, getInitials } from '@/lib/utils'
import { usePathname } from 'next/navigation'
import { Avatar, AvatarFallback } from '@/components/ui/avatar'
import { Session } from 'next-auth'

const Sidebar = ({ session }: { session: Session }) => {
	const pathname = usePathname()

	return (
		<div className='sticky left-0 top-0 flex h-dvh flex-col justify-between bg-white px-5 pb-5 pt-10'>
			<div>
				<div className='flex flex-row items-center gap-2 border-primary-admin/20 pb-10 max-md:justify-center'>
					<BookOpen size={32} />
					<h1 className='text-2xl font-semibold text-primary-admin max-md:hidden'>
						UniBook
					</h1>
				</div>
				<div className='mt-10 flex flex-col gap-5'>
					{adminSideBarLinks.map((link) => {
						const isSelected = pathname === link.href
						return (
							<Link
								key={link.text}
								href={link.href}
								className={cn(
									'flex flex-row items-center w-full gap-2 rounded-lg px-5 py-3.5 max-md:justify-center',
									isSelected && 'bg-green-900 shadow-sm'
								)}
							>
								<div className='relative size-5'>
									<DynamicIcon
										name={link.icon}
										size={20}
										strokeWidth={1.5}
										color={isSelected ? 'white' : 'black'}
									/>
								</div>
								<p
									className={cn(
										'text-base font-medium max-md:hidden',
										isSelected ? 'text-white' : 'text-black'
									)}
								>
									{link.text}
								</p>
							</Link>
						)
					})}
				</div>
			</div>
			<div className='my-8 flex w-full flex-row items-center gap-2 rounded-lg px-6 py-2 shadow-sm max-md:px-2'>
				<Avatar>
					<AvatarFallback className='bg-green-300'>
						{getInitials(session?.user?.name ?? 'Test User')}
					</AvatarFallback>
				</Avatar>
				<div className='flex flex-col max-md-hidden'>
					<p className='text-semibold text-dark-200'>{session?.user?.name}</p>
					<p className='text-xs text-light-500'>{session?.user?.email}</p>
				</div>
			</div>
		</div>
	)
}
export default Sidebar
