...
class User(db.Model):
    ...
    username: so.Mapped[str] = so.mapped_column(sa.String(32), index=True, nullable=False, unique=True)
    full_name: so.Mapped[str] = so.mapped_column(sa.String(255), nullable=False)
    email: so.Mapped[str] = so.mapped_column(sa.String(64), index=True, nullable=False, unique=True)
    ...
    created_at: so.Mapped[sa.DateTime] = so.mapped_column(sa.DateTime, server_default=sa.func.now())
